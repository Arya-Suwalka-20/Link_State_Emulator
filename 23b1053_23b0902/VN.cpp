...
string lsp_to_string(const LSP& l) {
    string s = "";
    s += l.origin;
    s += "(seq " + to_string(l.seq) + "): ";
    bool f = true;
    for (auto& p : l.links) {
        if (!f) s += ", ";
        s += string(1,p.first) + "=" + to_string(p.second);
        f = false;
    }
    return s;
}

void print_topology_matrix(const map<char, pair<uint32_t, vector<pair<char,int>>>>& lsp_db) {
    map<char, vector<pair<char,int>>> adj;
    set<char> all_nodes;
    for (auto& x : lsp_db) {
        all_nodes.insert(x.first);
        for (auto& p : x.second.second)
            all_nodes.insert(p.first);
    }
    for (char n : all_nodes) adj[n] = {};
    for (auto& x : lsp_db)
        adj[x.first] = x.second.second;

    vector<char> nodes(all_nodes.begin(), all_nodes.end());
    ...
    for (int i = 1; i < len; i++) {
        if (buf[i] == '(') {
            ...
            while (i < len && buf[i] != ')') {
                ...
                if (buf[i] != ' ') {
                    if (f == 0) nm += buf[i];
                    else if (f == 1) ip += buf[i];
                    else if (f == 2) port += buf[i];
                    else if (f == 3) cost += buf[i];
                }
                i++;
            }
            if (!nm.empty()) {
...
int main() {
    ...
    if (n <= 0) { cerr << "No LINK-STATE from ON\n"; return 0; }
    buffer[n] = '\0';
    ...
    char myName = '?';
    for (auto& nb : neighbors) if (nb.cost == 0) myName = nb.name;
    if (myName == '?') cerr << "Could not determine my node name from LINK-STATE!\n";
    ...
    while (running) {
        ...
        if (rv == 0) { // timeout: send periodic LSP
            LSP l; l.origin = myName; l.seq = seq_no++;
            for (auto& nb : neighbors) l.links.push_back({nb.name, nb.cost});
            vector<uint8_t> pkt = serialize_lsp(l);

            cout << "## sending messages to: ";
            for (auto& nb : neighbors) {
                if (nb.cost == 0) continue; // skip self
                sockaddr_in d{}; d.sin_family=AF_INET; d.sin_port=htons(nb.port);
                inet_pton(AF_INET, nb.ip.c_str(), &d.sin_addr);
                sendto(udp_sock, pkt.data(), pkt.size(), 0, (sockaddr*)&d, sizeof(d));
                cout << nb.name << " ";
            }
            cout << "### " << lsp_to_string(l) << " ###\n";
